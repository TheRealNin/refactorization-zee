...
function CommandStructure:UpdateCommanderLogin(force)
	self:CheckCommanderLogin(force) -- Stub to preserve backwards compatability with console command
end

function CommandStructure:CheckCommanderLogin(force)

	if Server then
		if self.occupied and self.commanderId == Entity.invalidId or force then
            ...
            if not GetGamerules():GetPlayerBannedFromCommand(playerSteamId) then
                ...
                if GetGamerules():OnCommanderLogin(self, player) then
                    ...
                    if not self.occupied and self.loginAllowed then
                        ...
                        self.occupied = true
                        csUseSuccess = true
						self:AddTimedCallback(CheckForLogin, self:isa("Hive") and 0 or kCheckForLoginTimer)
                        -- TODO: trigger client side in OnTag
...
function CommandStructure:OnTag(tagName)
    ...
    if tagName == "closed" then
        self:CheckCommanderLogin(false)
